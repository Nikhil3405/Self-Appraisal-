...
export async function POST(req: Request) {
  try {
    ...
    const { employeeId, category = "category-1", academicYear, data } = body;
...
export async function GET(req: Request) {
  try {
    ...
    const category = searchParams.get("category") || "category-1";
...
export async function DELETE(req: Request) {
  try {
    await connectToDatabase();
    const body = await req.json();
    const { employeeId, category = "category-1", academicYear } = body;
